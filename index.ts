...
const searchAndProcess = async (
  ...
  await generateText({
    ...
    tools: {
      searchWeb: tool({
        description:
          "Search the web for the most relevant results for the given query",
        parameters: z.object({
      ...
      evaluate: tool({
        description:
          "Evaluate the relevance of the given search results to the query",
        parameters: z.object({}),
        execute: async () => {
          ...
          return evaluation === "irrelevant"
            ? "Search results are irrelevant. Please search again."
            : "Search results are relevant. End research.";
        },
      }),
    },
  });
...
export const deepResearch = async (
  ...
  for (const query of queries) {
    ...
    for (const result of searchResults) {
      console.log(
        `Generating learning and follow-up question for ${result.url}`
      );
